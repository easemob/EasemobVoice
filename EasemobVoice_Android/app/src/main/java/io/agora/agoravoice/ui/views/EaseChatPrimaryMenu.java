...
//            buttonPressToSpeak.setVisibility(GONE);
//            showNormalFaceImage();
            ToastUtil.showShortToast(getContext(),"启动发送照片");
        }else {
            showTextStatus();
        }
        checkMenuType();
        EMLog.d("wyz","showMoreStatus()  (listener != null) ： "+(listener != null));
        if(listener != null) {
        ...
        if(id == R.id.btn_send) {//发送
            if(listener != null) {
                ...
                if (TextUtils.isEmpty(s)) {
                    ToastUtil.showShortToast(activity,"发送内容不能为空!");
                    return;
                }
                editText.setText("");
...
//            showVoiceStatus();
            ToastUtil.showShortToast(activity,"功能开发中");
        }else if (id == R.id.btn_set_mode_keyboard) {//切换到文本模式
            showTextStatus();
        }else if (id == R.id.btn_more) {//切换到更多模式
//            showMoreStatus();
            if(listener != null) {
                listener.onChatExtendMenuItemClick(0,null);
            }
...
//        inputManager.showSoftInput(editText,InputMethodManager.SHOW_FORCED);
    ...
    public void afterTextChanged(Editable s) {
        Log.d("TAG", this.getClass().getSimpleName() + " afterTextChanged s:"+s);
    }
}
