...
public class ProxyManager implements BusinessProxyListener {
    ...
    private ArrayList<ImServiceListener> mImServiceListeners;
    ...
    public interface ImServiceListener {
        ...
        void onImServiceFailed(int type, int code, String message);
    }
    ...
    public ProxyManager(@NonNull BusinessProxyContext context) {
        ...
        mImServiceListeners = new ArrayList<>();
        ...
        mImManager = new IMManager(mBusinessProxy);
    ...
    public void addImServiceListener(ImServiceListener listener) {
        if (!mImServiceListeners.contains(listener)) {
            mImServiceListeners.add(listener);
        }
    }

    public void removeImServiceListener(IMServiceListener listener) {
        mImServiceListeners.remove(listener);
    }
    ...
    public void onBusinessFail(int type, int code, String message) {
        switch (type) {
            ...
            case BusinessType.LEAVE_ROOM:
                ...
                break;
            case BusinessType.GET_ADMIN_TOKEN:
            case BusinessType.CREATE_CHATROOM:
                for (ImServiceListener listener : mImServiceListeners) {
                    listener.onImServiceFailed(type, code, message);
                }
                break;
        }
    }
    ...
    public void onGetAdminTokenSuccess(String token) {
        for (ImServiceListener listener : mImServiceListeners) {
            listener.onGetAdminTokenSuccess(token);
        }
    }
    ...
    public void onCreateChatRoomSuccess(String imRoomID) {
        for (ImServiceListener listener : mImServiceListeners) {
            listener.onCreateChatRoomSuccess(imRoomID);
        }
    }
