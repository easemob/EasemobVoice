...
public class PreferenceManager {
	...
	private static String SHARED_KEY_RTC_SAVE_USERID = "shared_key_rtc_use_id";
	private static String SHARED_KEY_ADMIN_TOKEN = "shared_key_admin_token";
	private static String SHARED_KEY_USER_TOKEN = "shared_key_user_token";
	...
	public synchronized static PreferenceManager getInstance() {
		if (mPreferencemManager == null) {
			...
			if(!TextUtils.isEmpty(currentUser)) {
			    init(AgoraApplication.getInstance(), currentUser);
			}
			if(mPreferencemManager == null) {
	...
	public static void setCanRegister(boolean canRegister) {
		getDefaultSp(AgoraApplication.getInstance());
	...
	//保存 聊天室ID对应关系：roomId app的roomId，imRoomId即时通信聊天室ID
	public void saveUserObjId(String userId,String objId) {
		editor.putString(userId, objId);
		editor.apply();
	}

	public String getUserObjId(String userId) {
	...
	public void saveAdminToken(String token) {
		...
		editor.putString(SHARED_KEY_ADMIN_TOKEN, token);
		editor.apply();
	}

	public String getAdminToken() {
		return mSharedPreferences.getString(SHARED_KEY_ADMIN_TOKEN, "");
	}
