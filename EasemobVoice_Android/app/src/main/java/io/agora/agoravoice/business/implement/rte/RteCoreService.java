...
public class RteCoreService implements CoreService,
    ...
    public void disableLocalAudio() {
        ...
        mLocalUser.unPublishLocalMediaTrack(mLocalUser.getStreamId(),
            ...
            @Override
            public void success(@Nullable Void param) {
                Logging.w("disable local audio success");
            }

            @Override
            public void fail(@NotNull AgoraRteError error) {
                Logging.w("disable local audio fails " +
                        error.getCode() + " " + error.getMessage());
            }
        });
    }
    ...
    public void muteLocalAudio(boolean muted) {
        if (mLocalUser != null) {
            if (muted) {
                mLocalUser.muteLocalMediaStream(mLocalUser.getStreamId(), AgoraRteMediaStreamType.audio);
                mLocalUser.muteLocalMediaStream$rte_debug(AgoraRteMediaStreamType.audio,true);
            } else {
                mLocalUser.unMuteLocalMediaStream(mLocalUser.getStreamId(), AgoraRteMediaStreamType.audio);
                mLocalUser.unsubscribeRemoteStream(mLocalUser.getStreamId(), AgoraRteMediaStreamType.audio);
            }
        } else {
