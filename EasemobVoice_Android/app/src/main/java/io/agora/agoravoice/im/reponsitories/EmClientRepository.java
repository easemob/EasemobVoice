...
public class EmClientRepository extends BaseEMRepository {
    ...
    public LiveData<Resource<User>> loginIm(User user) {
        return new NetworkOnlyResource<User,User>() {
            ...
            @Override
            protected void createCall(ResultCallBack<LiveData<User>> callBack) {
                runOnIOThread(() -> {
                    if(isLoggedIn()) {
     ...
     * @return
     */
    public LiveData<Resource<EMChatRoom>> joinChatRoom(String roomId){
        return new NetworkOnlyResource<EMChatRoom, EMChatRoom>() {
            ...
            protected void createCall(@NonNull ResultCallBack<LiveData<EMChatRoom>> callBack) {
                getChatRoomManager().joinChatRoom(roomId, new EMValueCallBack<EMChatRoom>() {
                    @Override
                    public void onSuccess(EMChatRoom emChatRoom) {
                        callBack.onSuccess(createLiveData(emChatRoom));
                    }
    ...
    public LiveData<Resource<EMChatRoom>> leaveChatRoom(String userName) {
        return new NetworkOnlyResource<EMChatRoom, EMChatRoom>() {
            @Override
            protected void createCall(@NonNull ResultCallBack<LiveData<EMChatRoom>> callBack) {
                getChatRoomManager().leaveChatRoom(userName);
            }
        }.asLiveData();
    }
