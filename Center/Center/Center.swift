...
private extension Center {
    ...
    func register(success: ((BasicUserInfo) -> Void)?) {
        ...
        let successCallback: DicEXCompletion = { (json: ([String: Any])) throws in
            ...
            if let success = success {
                // 声网注册成功以后，接着注册环信相关
                if let err = EMClient.shared().register(withUsername: userId, password: BasicUserInfo.password) {
    ...
    func login(user: BasicUserInfo, success: Completion = nil) {
        ...
        let successCallback: DicEXCompletion = { [unowned self] (json: ([String: Any])) throws in
            ...
            self.rteLogin(userId: user.userId) {
                // 环信登录
                if !EMClient.shared().isLoggedIn {
                    if let err = EMClient.shared().login(withUsername: user.userId, password: BasicUserInfo.password) {
                        debugPrint("em account login failed. \(err)")
                        return
                    }
                }
                success?()
