...
class BottomToolsViewController: RxViewController {
    ...
    override func viewDidLoad() {
        ...
        placeholderButton.setAttributedTitle(NSAttributedString(string: LiveVCLocalizable.chatInputPlaceholder(),
        ...
        placeholderButton.addTarget(self, action: #selector(showInputBar(sender:)), for: .touchUpInside)
        placeholderButton.backgroundColor = .clear
        textInput.addSubview(placeholderButton)
    ...
    @objc
    func showInputBar(sender: UIButton) {
        guard let chatRoomVC = parent as? ChatRoomViewController else {
            return
        }
        chatRoomVC.showMaskView() {
            chatRoomVC.toggleInputBar(false)
        }
        chatRoomVC.toggleInputBar(true)
    }
    ...
    override func viewDidLayoutSubviews() {
        ...
        textInput.frame = CGRect(x: space,
                                 ...
                                 height: viewHeight)
        for btn in textInput.subviews {
            if btn is UIButton {
                btn.frame = CGRect(x: 0, y: 0, width: textInput.frame.size.width - 15, height: viewHeight)
                break
            }
        }
