...
extension Fillet {
    func drawFillet() {
        let bezier = UIBezierPath(arcCenter: CGPoint(x: filletRadius,
                                                     y: filletRadius),
                                  radius: filletRadius,
        ...
        let rightTopPoint = CGPoint(x: bounds.width - filletRadius,
                                    y: 0)
        bezier.addLine(to: rightTopPoint)
        
        bezier.addArc(withCenter: CGPoint(x: bounds.width - filletRadius,
                                          y: filletRadius),
                      radius: filletRadius,
        ...
        let rightBottomPoint = CGPoint(x: bounds.width,
                                       y: bounds.height - filletRadius)
        ...
        let leftBottomPoint = CGPoint(x: filletRadius,
                                      y: self.bounds.height)
        bezier.addLine(to: leftBottomPoint)
        
        bezier.addArc(withCenter: CGPoint(x: filletRadius,
                                          y: self.bounds.height - filletRadius),
                      radius: filletRadius,
                      startAngle: CGFloat.pi * 0.5,
                      endAngle: CGFloat.pi,
                      clockwise: true)
        let leftTopPoint = CGPoint(x: 0,
                                   y: filletRadius)
...
class FilletView: UIView, Fillet {
    ...
    var filletRadius: CGFloat = 0.0 {
        didSet {
            guard filletRadius <= bounds.height * 0.5 else {
                filletRadius = bounds.height * 0.5
                return
            }
            setNeedsDisplay()
        }
