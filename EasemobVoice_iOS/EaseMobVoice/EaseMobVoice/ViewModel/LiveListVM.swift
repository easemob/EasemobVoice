...
struct Room {
    ...
    init(dic: StringAnyDic) throws {
        let json = JSON(dic)
        self.name = json["roomName"].stringValue
        self.roomId = json["roomId"].stringValue
        
        self.personCount = json["onlineUsers"].intValue
        
        self.imageURL = json["thumbnail"].stringValue
        
        let ownerJson = try dic.getDictionaryValue(of: "ownerUserInfo")
        self.owner = try LiveRoleItem(dic: ownerJson)
        
        let index = json["backgroundImage"].int ?? 1
        let images = Center.shared().centerProvideImagesHelper()
        self.image = images.getRoomPreview(index: index)
    }
}
