...
extension LiveSession {
    static func create(roomName: String,
        ...
        client.request(task: task, success: ArResponse.json({ (json) in
            ...
            // 在这里需要创建一个对应的聊天室
            EMServer.shared.createChatRoom(roomId)
    ...
    func join(success: ((LiveSession) -> Void)? = nil,
        ...
        EMServer.shared.fetchChatRoom(room.value.roomId) { [unowned self] id in
            EMClient.shared().roomManager.joinChatroom(id) { room, error in
                if let err = error {
                    debugPrint("join chat room failed \(err.errorDescription!)")
                } else {
                    
                    debugPrint("join chatroom succeed")
    ...
    func leave() {
        ...
        EMServer.shared.fetchChatRoom(room.value.roomId) { id in
            var error: EMError!
            EMClient.shared().roomManager.leaveChatroom(id, error: &error)
            if let error = error {
                debugPrint(error)
            }
        }
