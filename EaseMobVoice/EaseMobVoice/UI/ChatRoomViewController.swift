...
class ChatRoomViewController: MaskLogViewController, LiveViewController {
    ...
    lazy var inputBar: EaseMobInputBar = EaseMobInputBar()
    var needShowInputBar: Bool = false
    ...
    override var canBecomeFirstResponder: Bool {
        return needShowInputBar
    }
    
    func toggleInputBar(_ show: Bool) {
        // 以下调用时间，不能随便改动
        if show {
            needShowInputBar = true
            becomeFirstResponder()
            inputBar.delegate = self
            inputBar.imageDelegate = self
            inputBar.inputTextView.becomeFirstResponder()
        } else {
            if !self.maskView.isHidden {
                self.hiddenMaskView()
            }
            needShowInputBar = false
    ...
    var liveSession: LiveSession! {
        didSet {
            ...
            EMServer.shared.fetchChatRoom(liveSession.room.value.roomId) { [weak self] id in
                self?.chatVM.roomId = id
                s.signal()
            }
            s.wait()
        }
...
//        chatInput()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "ChatViewController":
            ...
            self.chatVC = vc
            self.chatVC?.chatVM = chatVM
            chatVM.collectionView = self.chatVC?.messagesCollectionView
        default:
            break
        }
    }
...
extension ChatRoomViewController: InputBarAccessoryViewDelegate {
    ...
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        self.toggleInputBar(false)
        inputBar.inputTextView.text = ""
        guard let name = CurrentUser.local()?.info.value.name else {
            return
        }
        chatVM.newText(text, name: name)
    }
}

extension ChatRoomViewController: EaseMobInputBarImageDelegate {
    
    func inputBar(_ inputBar: InputBarAccessoryView, didSelect images: [UIImage]) {
        guard let name = CurrentUser.local()?.info.value.name else {
            return
        }
        chatVM.newImages(images, name: name)
    }
}
